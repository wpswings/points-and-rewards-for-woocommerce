...
class Points_Rewards_For_WooCommerce_Public
{
	...
	public function wps_wpr_woocommerce_order_status_changed($order_id, $old_status, $new_status)
	{
		...
		if (in_array($old_status, $mwb_wpr_array, true) && ('cancelled' === $new_status || 'refunded' === $new_status)) {
			error_log('Refunde and cancelled points on cart applied here.');
			$order = wc_get_order($order_id);
			$user_id = absint($order->get_user_id());
				...
				if ($wps_wpr_order_points) {
					...
					if ($get_min_redeem_req <= $get_points) {
						...
						<div class="wps_wpr_apply_custom_points">
							<input type="number" min="0" name="wps_cart_points" class="input-text" id="wps_cart_points" value=""
								placeholder="<?php esc_attr_e('Points', 'points-and-rewards-for-woocommerce'); ?>" />
							<button class="button wps_cart_points_apply" name="wps_cart_points_apply" id="wps_cart_points_apply"
								value="<?php esc_html_e('Apply Points', 'points-and-rewards-for-woocommerce'); ?>"
								data-id="<?php echo esc_html($user_id); ?>"
								data-order-limit="0"><?php esc_html_e('Apply Points', 'points-and-rewards-for-woocommerce'); ?></button>
							<p class="wps_wpr_restrict_user_message">
								<?php esc_html_e('Your available points:', 'points-and-rewards-for-woocommerce'); ?>
								<?php echo esc_html($get_points); ?>
							</p>
						...
						<div class="wps_wpr_apply_custom_points">
							<input type="number" min="0" name="wps_cart_points" class="input-text" id="wps_cart_points" value=""
								placeholder="<?php esc_attr_e('Points', 'points-and-rewards-for-woocommerce'); ?>" readonly />
							<button class="button wps_cart_points_apply" name="wps_cart_points_apply" id="wps_cart_points_apply"
								value="<?php esc_html_e('Apply Points', 'points-and-rewards-for-woocommerce'); ?>"
								data-id="<?php echo esc_html($user_id); ?>" data-order-limit="0"
								disabled><?php esc_html_e('Apply Points', 'points-and-rewards-for-woocommerce'); ?></button>
							<p><?php esc_html_e('You require :', 'points-and-rewards-for-woocommerce'); ?>
								<?php echo esc_html($extra_req); ?>
							</p>
	...
	public function wps_wpr_woocommerce_before_cart_contents()
	{
		...
		if ((1 == $wps_wpr_custom_points_on_cart || 1 === $wps_wpr_custom_points_on_checkout) && !empty($user_id)) {
			if ($wps_wpr_show_redeem_notice) {
				?>
				<div class="woocommerce-message wps_wpr_cart_redemption__notice" id="wps_wpr_order_notice"
					style="background-color: <?php echo esc_html($wps_wpr_notification_color); ?>;">
					<?php esc_html_e('Here is the Discount Rule for Applying your Points to Cart Total', 'points-and-rewards-for-woocommerce'); ?>
		...
		if ($this->is_order_conversion_enabled()) {
			if ($wps_wpr_per_currency_discount_notice) {
				...
				?>
				<div class="woocommerce-message" id="wps_wpr_order_notice"
					style="background-color: <?php echo esc_html($wps_wpr_notification_color); ?>">
					<?php
		...
		if (empty($wps_wpr_rewards_points_awarded_check)) {
			...
			if (1 === $wps_wpr_enable_order_rewards_settings) {
				if (1 === $wps_wpr_enable_to_show_order_reward_message) {
					...
					<div class="woocommerce-message" id="wps_wpr_order_notice"
						style="background-color: <?php echo esc_attr($wps_wpr_notification_color); ?>">
						<p style="background-color: <?php echo esc_attr($wps_wpr_notification_color); ?>">
							<?php echo wp_kses_post($wps_wpr_number_order_rewards_messages); ?>
						</p>
					</div>
					<?php
				}
		...
		if (1 == $wps_wpr_custom_points_on_checkout && 1 == $wps_wpr_custom_points_on_cart) {
			if ('Avada' == wp_get_theme()->Name) {
				...
				<div class="wps_wpr_avada_wrap checkout_coupon">
					<h2 class="promo-code-heading fusion-alignleft">
						<?php esc_html_e('Have A Points?', 'points-and-rewards-for-woocommerce'); ?>
					</h2>
				...
				if ($get_min_redeem_req <= $get_points) {
					...
					<div class="custom_point_checkout woocommerce-info wps_wpr_checkout_points_class">
						...
						<button class="button wps_cart_points_apply" name="wps_cart_points_apply" id="wps_cart_points_apply"
							...
							data-id="<?php echo esc_html($user_id); ?>"
							data-order-limit="0"><?php esc_html_e('Apply Points', 'points-and-rewards-for-woocommerce'); ?></button>
						<p><?php echo esc_html($get_points) . esc_html__(' Points', 'points-and-rewards-for-woocommerce') . ' = ' . wp_kses(wc_price(apply_filters('wps_wpr_show_conversion_price', $conversion)), $this->wps_wpr_allowed_html()); ?>
						</p>
					</div>
					<?php
				} else {
					...
					<div class="custom_point_checkout woocommerce-info wps_wpr_checkout_points_class">
						...
						<p><?php esc_html_e('You require :', 'points-and-rewards-for-woocommerce'); ?>
							<?php echo esc_html($extra_req); ?>
							<?php esc_html_e('more points to get redeem', 'points-and-rewards-for-woocommerce'); ?>
						</p>
					</div>
					<?php
				}
		...
		if (is_plugin_active('wallet-system-for-woocommerce/wallet-system-for-woocommerce.php') && !empty(get_option('wps_wsfw_enable', ''))) {
			...
			if ($wps_wallet_enable && !empty($wps_req_points)) {
				...
				<div class="wps_wpr_wallet_conversion_wrap wps_wpr_main_section_all_wrap">
					<p class="wps_wpr_heading">
						<?php echo esc_html__('Convert Points to Currency Wallet Conversion', 'points-and-rewards-for-woocommerce'); ?>
					</p>
					<fieldset class="wps_wpr_each_section">
						<p>
						...
						<form id="points_wallet" enctype="multipart/form-data" action="" method="post">
							...
							<p class="wps-wpr_enter-points-wrap">
								<input type="number"
									placeholder="<?php esc_html_e('Enter your points:', 'points-and-rewards-for-woocommerce'); ?>"
									class="woocommerce-Input woocommerce-Input--number input-number" name="wps_custom_number" min="1"
									id="wps_custom_wallet_point_num" style="width: 160px;">

								<input type="button" name="wps_wpr_custom_wallet" id="wps_wpr_custom_wallet"
									class="wps_wpr_custom_wallet button"
									value="<?php esc_html_e('Redeem to Wallet', 'points-and-rewards-for-woocommerce'); ?>"
									data-id="<?php echo esc_html($user_id); ?>">
							</p>
						</form>
					</fieldset>
					...
					if ($get_min_redeem_req <= $get_points) {
						...
						<div class="custom_point_checkout woocommerce-info wps_wpr_checkout_points_class">
							...
							<button class="button wps_cart_points_apply" name="wps_cart_points_apply" id="wps_cart_points_apply"
								...
								data-id="<?php echo esc_html($user_id); ?>"
								data-order-limit="0"><?php esc_html_e('Apply Points', 'points-and-rewards-for-woocommerce'); ?></button>
							<p><?php echo esc_html($get_points) . esc_html__(' Points', 'points-and-rewards-for-woocommerce') . ' = ' . wp_kses(wc_price($conversion), $this->wps_wpr_allowed_html()); ?>
							</p>
						</div>
						<?php
					} else {
		...
		if (wps_wpr_check_is_subscription_plugin_active()) {
			...
			if ('1' == $wps_wpr_enable__renewal_message_settings) {
				...
				<div class="wps_wpr_subscription_notice_wrap">
					<p class="wps_wpr_heading">
						<?php echo esc_html__('Subscription Renewal Points Message :', 'points-and-rewards-for-woocommerce'); ?>
					</p>
					<?php
			...
			if ($this->wps_wpr_win_wheel_page_display()) {
						...
						<div class="wps_wpr_wheel">
							...
							if (!empty($wps_wpr_enter_segment_name) && is_array($wps_wpr_enter_segment_name)) {
								foreach ($wps_wpr_enter_segment_name as $key => $game_value) {
									?>
									<div class="wps_wpr_number" style="--color:<?php echo esc_html($wps_wpr_enter_segment_color[$key]); ?>">
										<input type="text"
											style="--label-font-size:<?php echo esc_html($wps_wpr_enter_sgemnet_font_size[$key]); ?>px"
											value="<?php echo esc_html($wps_wpr_enter_segment_name[$key]); ?>"
											data-attr="<?php echo esc_html($wps_wpr_enter_segment_points[$key]); ?>" readonly />
									</div>
									<?php
								}
							}
					...
					<div class="wps_wpr_container-popup">
						...
						<div class="wps_wpr_container-popup-in">
							...
							<div class="wps_wpr_container-popup-content">
								<div class="wps_wpr_container-popup-val">
									<?php esc_html_e('Hurray! You have got', 'points-and-rewards-for-woocommerce'); ?> <span
										class="wps_wpr-val"></span>
								</div>
								<input type="hidden" class="wps_wpr_user_claim_points" value="">
							</div>
							<button
								class="wps_wpr_container-popup-claim"><?php esc_html_e('Claim Now', 'points-and-rewards-for-woocommerce'); ?></button>
				...
				<div class="wps_wpr_guest_user_main_wrapper">
					<div class="wps_wpr_guest_user_contain_wrapper">
						...
						<div class="wps_wpr_guest_popup_title">
							<?php esc_html_e('Oops! You are not logged in.', 'points-and-rewards-for-woocommerce'); ?>
						</div>
						<div class="wps_wpr_para">
							<?php esc_html_e('To play the game, please ', 'points-and-rewards-for-woocommerce'); ?><a
								href="<?php echo esc_url(get_permalink(get_option('woocommerce_myaccount_page_id'))); ?>"><?php esc_html_e('click here', 'points-and-rewards-for-woocommerce'); ?></a><?php esc_html_e(' to login.', 'points-and-rewards-for-woocommerce'); ?>
						</div>
					</div>
				</div>
				<?php
		...
		if ('yes' === $wps_wpr_enable_user_badges_settings) {
			if ('yes' === $wps_wpr_enable_to_show_bades) {
				if (!empty($wps_wpr_assigned_badges_level_name)) {
					...
					?>
					<div id="wps-par__badge-wrap"
						class="wps-par__badge-wrap wps-badge-<?php echo esc_html($wps_wpr_choose_badges_position); ?>">
						...
						<div class="wps-par__badge-label">
							<div class="wps_wpr_account_page_badge_name_wrapper">
								<?php echo esc_html($wps_wpr_assigned_badges_level_name); ?>
							</div>
							<span>
	...
	public function wps_wpr_verify_cart_page_nonce()
	{
		...
		<input type="hidden" name="wps_wpr_verify_cart_nonce"
			value="<?php echo esc_html(wp_create_nonce('wps-cart-nonce')); ?>">
		<?php
	}

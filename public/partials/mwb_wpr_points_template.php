...
$mwb_text_points_value = isset($general_settings['mwb_wpr_general_text_points']) ? $general_settings['mwb_wpr_general_text_points'] : __('My Points',MWB_RWPR_Domain);
	...
	<?php if(isset($mwb_user_point_expiry) && !empty($mwb_user_point_expiry) && $get_points > 0){
		$mwb_wpr_points_exp_onmyaccount = get_option('mwb_wpr_points_exp_onmyaccount','off');
			if($mwb_wpr_points_exp_onmyaccount == 'on'){
				...
				echo '<p class=mwb_wpr_points_expiry> '.__('Get Expired: ',MWB_RWPR_Domain).$expirydate_format.'</p>';
			}
		}
		?>
</div>		
	<span class="mwb_wpr_view_log">
		<a href="<?php echo get_permalink().'view-log/'.$user_id ?>"><?php _e("View Point Log",MWB_RWPR_Domain);?></a>
	</span>
<?php
if(isset($mwb_ways_to_gain_points_value) && !empty($mwb_ways_to_gain_points_value)){ ?>
	<div class ="mwb_ways_to_gain_points_section">
	<p class="mwb_wpr_heading"><?php echo __( 'Ways to gain more points:', MWB_RWPR_Domain ); ?></p>
			<?php 
...
if($mwb_wpr_mem_enable)
{
	...
	$mwb_wpr_membership_roles = isset($membership_settings_array['membership_roles']) && !empty($membership_settings_array['membership_roles']) ? $membership_settings_array['membership_roles'] : array();
?>	
	<p class="mwb_wpr_heading"><?php _e('Membership List', MWB_RWPR_Domain); ?></p>
		<?php if(isset($user_level) && !empty($user_level)) 
		{
			?>
			<span class="mwb_wpr_upgrade_level"><?php _e('Your level has been upgraded to ', MWB_RWPR_Domain); echo $user_level; ?></span>
		<?php 
		}
		?>	
			<table class="woocommerce-MyAccount-points shop_table my_account_points account-points-table mwb_wpr_membership_with_img">
				<thead>
					<tr>
						<th class="points-points">
							<span class="nobr"><?php echo __( 'Level', MWB_RWPR_Domain ); ?></span>
						</th>
						<th class="points-code">
							<span class="nobr"><?php echo __( 'Required Points', MWB_RWPR_Domain ); ?></span>
						</th>
					</tr>
				</thead>
				<tbody>
				...
				{?>
				<tr>
					<td><?php echo $role.'<br/><a class = "mwb_wpr_level_benefits" data-id = "'.$role.'" href="javascript:;">'.__( 'View Benefits',MWB_RWPR_Domain ).'</a>'; ?></td>
							...
							<div class="mwb_wpr_popup_content_section">
								<div class="mwb_wpr_popup_content">
									<div class="mwb_wpr_popup_notice_section">					
										<p>
											<span class="mwb_wpr_intro_text"><?php _e('You will get ',MWB_RWPR_Domain); echo $values['Discount'];_e('% discount on below products or categories',MWB_RWPR_Domain);?>
											</span>
											<span class="mwb_wpr_close">
												<a href="javascript:;"><img src="<?php echo MWB_RWPR_DIR_URL;?>public/images/cancel.png" alt=""></a>
											</span>
										</p>
									</div>
					...
					if($role == $user_level){
						echo '<img class="mwb_wpr_tick" src = "'.MWB_RWPR_DIR_URL.'public/images/tick.png">';
					}
					?></td>
				</tr>	
				<?php
...
do_action('mwb_wpr_add_coupon_generation',$user_id);
 ?>
<br>
<?php do_action('mwb_wpr_list_coupons_generation',$user_id);
